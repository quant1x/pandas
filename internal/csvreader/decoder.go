...
func getTagValue(fieldTag reflect.StructTag, tagName string) (value string) {
	value = fieldTag.Get(tagName)
	value = strings.TrimSpace(value)
	if len(value) == 0 {
		return
	}
	tagOpts := strings.Split(value, ",")
	value = tagOpts[0]
	value = strings.TrimSpace(value)
	return value
}

// 兼容tag, 支持csv和dataframe两种, 为和pandas对齐
func compatTagValue(fieldTag reflect.StructTag) string {
	tagValue := getTagValue(fieldTag, readerTagCsv)
	if len(tagValue) > 0 {
		return tagValue
	}
	tagValue = getTagValue(fieldTag, readerTagDataframe)
	return tagValue
}

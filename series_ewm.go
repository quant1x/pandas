...
func (w ExponentialMovingWindow) applyMean(data Series, alpha num.DType) Series {
	if w.Adjust {
...
func (w ExponentialMovingWindow) adjustedMean(data Series, alpha num.DType, ignoreNA bool) {
	var (
		values           = data.Values().([]num.DType)
		weight num.DType = 1
		last             = values[0]
	)
...
func (w ExponentialMovingWindow) notAdjustedMean(data Series, alpha num.DType, ignoreNA bool) {
	hasCallback := false
	if num.DTypeIsNaN(alpha) {
		hasCallback = true
		alpha = w.Cb(0)
	}
	var (
